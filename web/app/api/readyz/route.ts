...
export async function GET() {
  ...
  const upstream = new URL("/readyz", apiBase);

  try {
    const { response, duration } = await fetchUpstream(upstream, {}, 4000);
    if (!response.ok) {
      console.warn(`proxy /api/readyz failed: ${response.status} in ${duration}ms`);
      return proxyJsonError("upstream error", response.status, upstream.href);
    }
    const payload = await response.json();
    return NextResponse.json(payload);
  } catch (err) {
    console.warn(`proxy /api/readyz error: ${(err as Error).message}`);
