...
export async function GET(
  ...
  const upstream = new URL(`/api/rollups/${context.params.id}`, apiBase);

  try {
    const { response, duration } = await fetchUpstream(upstream);
    if (!response.ok) {
      console.warn(
        `proxy /api/rollups/:id failed: ${response.status} in ${duration}ms`
      );
    ...
    console.warn(`proxy /api/rollups/:id error: ${(err as Error).message}`);
