...
export async function GET(
  ...
  const upstream = new URL(`/calls/${context.params.id}/rollup`, apiBase);

  try {
    const { response, duration } = await fetchUpstream(upstream);
    if (!response.ok) {
      console.warn(
        `proxy /api/calls/:id/rollup failed: ${response.status} in ${duration}ms`
      );
    ...
    console.warn(
      `proxy /api/calls/:id/rollup error: ${(err as Error).message}`
    );
