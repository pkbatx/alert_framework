...
export async function GET(
  ...
  const upstream = new URL(`/calls/${context.params.id}/metadata`, apiBase);

  try {
    const { response, duration } = await fetchUpstream(upstream);
    if (!response.ok) {
      console.warn(
        `proxy /api/calls/:id/metadata failed: ${response.status} in ${duration}ms`
      );
    ...
    console.warn(
      `proxy /api/calls/:id/metadata error: ${(err as Error).message}`
    );
